...
async function current(req){
  const x=session(req);
  if(!x)return{x:null,police:null,role:'citizen',admin:false,adminPerms:[]};
  let rows=[];
  try { rows=await readSheet(); } catch (e) { console.warn(`Police sheet unavailable during session lookup: ${e.message}`); }
  const police=rows.find(r=>r.discordId===String(x.id))||null;
  const r=role(police),admin=await isAdmin(x.id),adminPerms=await adminPermissions(x.id);
  return{x,police,role:r,admin,adminPerms};
}
...
app.get('/api/admin/state',async(q,s)=>{const c=await requirePerm(q,s,'view_dashboard');if(!c)return;try{let rows=[];try{rows=await readSheet();}catch(e){console.warn(`Police sheet unavailable for admin state: ${e.message}`);}const admins=await readAdmins();s.json({members:rows.map(r=>({...safe(r),role:role(r),admin:ENV_ADMIN_IDS.has(r.discordId)||admins.some(a=>a.discordId===r.discordId&&a.active)})),admins,permissions:PERMISSION_LABELS,hierarchy,me:{permissions:c.adminPerms,isSuperAdmin:c.adminPerms.length===ALL_PERMISSIONS.length}});}catch(e){s.status(503).json({error:e.message})}});
