...
app.get('/api/me',async(q,s)=>{const x=session(q);if(!x)return s.json({authenticated:false});try{const p=(await readSheet()).find(r=>r.discordId===String(x.id))||null,r=role(p);return s.json({authenticated:true,discord:x,police:safe(p),role:r,permissions:perms(r)})}catch(e){return s.status(503).json({authenticated:true,discord:x,syncError:e.message})}});
app.get('/api/academy/members',async(q,s)=>{const x=session(q);if(!x)return s.status(401).json({error:'UNAUTHENTICATED'});const rows=await readSheet(),me=rows.find(r=>r.discordId===String(x.id));if(role(me)==='citizen')return s.status(403).json({error:'OFFICER_ONLY'});s.json({updatedAt:cache.at,members:rows.map(safe)})});
