...
function current(req){const x=sess(req);if(!x)return{x:null,police:null,role:'citizen',admin:false,permissions:[]};const a=admin(x.id),isAdmin=ADMINS.has(id(x.id))||Boolean(a?.enabled);try{const rows=await withTimeout(police(),4000),p=rows.find(r=>id(r.discordId)===id(x.id))||null;return{x,police:p,role:role(p),admin:isAdmin,permissions:perms(x.id),sheet:true}}catch(e){return{x,police:null,role:'unknown',admin:isAdmin,permissions:perms(x.id),sheet:false,error:e.message}}}
...
app.get('/auth/discord',(_q,res)=>{if(!CLIENT_ID||!CLIENT_SECRET)return res.status(503).send('Discord OAuth is not configured yet.');const p=new URLSearchParams({client_id:CLIENT_ID,response_type:'code',redirect_uri:REDIRECT,scope:'identify guilds'});res.redirect(`https://discord.com/oauth2/authorize?${p}`)});
