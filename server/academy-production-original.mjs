...
app.patch('/api/admin/exams/:id/active',(req,res)=>saved(async()=>{const c=await requireAdmin(req,res,'manage_exams');if(!c)return;const e=data.exams.find(x=>String(x.id)===String(req.params.id));if(!e)return res.status(404).json({error:'EXAM_NOT_FOUND'});e.active=req.body?.active===true;audit(c,e.active?'ACTIVATE_EXAM':'DEACTIVATE_EXAM',e.id);if(supabaseActive)await saveAcademyData(data);else await persistExamStorage(['exams']);res.json({ok:true,exam:e})},req,res));
