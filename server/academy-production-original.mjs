...
app.get('/api/admin/activity-state',async(req,res)=>{const c=await requireAdmin(req,res,'view_activity_logs');if(!c)return;res.json({audit:Array.isArray(data.audit)?data.audit.slice(0,1000):[],loginLogs:Array.isArray(data.loginLogs)?data.loginLogs.slice(0,1000):[]})});
