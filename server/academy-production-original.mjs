...
app.post('/api/admin/badges',async(req,res)=>{try{const c=await requireAdmin(req,res,'manage_members');if(!c)return;const name=String(req.body?.name||'').trim().slice(0,80);const description=String(req.body?.description||'').trim().slice(0,240);const icon=String(req.body?.icon||'🏅').trim().slice(0,8);if(name.length<2)return res.status(400).json({error:'INVALID_BADGE'});const badge={id:'badge-'+Date.now()+'-'+crypto.randomBytes(3).toString('hex'),name,description,icon,createdAt:new Date().toISOString(),createdBy:String(c.x.id)};data.badges=Array.isArray(data.badges)?data.badges:[];data.badges.unshift(badge);audit(c,'CREATE_BADGE',badge.id,name);await save();res.json({ok:true,badge})}catch(e){console.error(e);res.status(503).json({error:'STORAGE_ERROR'})}});
