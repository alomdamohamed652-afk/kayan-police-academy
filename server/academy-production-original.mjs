...
app.post('/api/admin/exams',(req,res)=>saved(async()=>{const c=await requireAdmin(req,res,'manage_exams');if(!c)return;const e=cleanExam(req.body);validateExam(e);data.exams.unshift(e);audit(c,'CREATE_EXAM',e.id,`${e.title} · ${e.accessType}`);if(supabaseActive)await saveAcademyData(data);else await persistExamStorage(['exams']);res.json({ok:true,exam:e})},req,res));
