...
function publicExam(e){const questions=(Array.isArray(e?.questions)?e.questions:[]).map(cleanQuestion).filter(q=>q.text).map(q=>({id:q.id,text:q.text,type:q.type,options:q.options||[],required:q.required!==false,points:q.points}));return{...e,state:timeState(e?.startAt,e?.endAt),accessToken:undefined,allowedDiscordIds:undefined,questions:questions.map(q=>({...q,sectionId:String(q.sectionId||''),imageUrl:String(q.imageUrl||'').trim()||undefined}))}}
...
app.post('/api/exams/:id/submit',(req,res)=>saved(async()=>{
  ...
  const currentSection=currentSectionId?sections.find(sec=>String(sec.id)===currentSectionId):null;
  const gateFailed=Boolean(currentSection&&currentSection.gateQuestionId&&Array.isArray(currentSection.allowedAnswers)&&currentSection.allowedAnswers.length&& !currentSection.allowedAnswers.map(String).map(v=>v.trim()).filter(Boolean).includes(String(answers[currentSection.gateQuestionId]??'').trim()));
