...
function cleanQuestion(q){const type=['choice','yesno','text','info'].includes(q?.type)?q.type:'text';const options=Array.isArray(q?.options)?q.options.map(v=>String(v).trim()).filter(Boolean):[];let correct=q?.correct==null?'':String(q.correct);if(type==='choice'&&!options.includes(correct))correct='';if(type==='yesno'&&!['نعم','لا'].includes(correct))correct='';if(type==='text'||type==='info')correct='';const questionBankId=q?.questionBankId?String(q.questionBankId).trim():'';const imageUrl=String(q?.imageUrl||'').trim();const sectionId=String(q?.sectionId||'').trim();return{id:String(q?.id||`q-${Date.now()}-${Math.random().toString(36).slice(2,7)}`),text:String(q?.text||'').trim(),type,options,correct,required:q?.required!==false,points:Math.max(0,Number(q?.points??1)),...(questionBankId?{questionBankId}:{}),...(imageUrl?{imageUrl}:{}),...(sectionId?{sectionId}:{})}}
...
function scoreAttempt(e,answers){let earned=0,total=0;for(const q of e.questions||[]){const p=Math.max(0,Number(q.points??1));if(p>0)total+=p;if((q.type==='choice'||q.type==='yesno')&&String(answers?.[q.id]??'')===String(q.correct??''))earned+=p}return total?Math.round(earned/total*100):0}
