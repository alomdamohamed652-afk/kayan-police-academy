...
const separate=await loadExamStorage(s);if(separate.hasSeparate){if(separate.exams)data.exams=separate.exams;if(separate.results)data.examResults=separate.results;if(separate.attempts)data.examAttempts=separate.attempts}else{await saveExamStorage()}storageReady=true;lastStorageError='';let changed=false;const superId=SUPER_ADMIN_ID;const beforeEnv=data.admins.length;data.admins=data.admins.filter(a=>a?.source!=='environment'||id(a.discordId)===superId);if(data.admins.length!==beforeEnv)changed=true;if(superId&&!data.admins.some(a=>id(a.discordId)===superId)){data.admins.push({discordId:superId,name:'Super Admin',permissions:ALL,enabled:true,createdAt:new Date().toISOString(),source:'environment'});changed=true}if(changed)await save();console.log(`Google DATA storage ready (${DATA_SHEET})`)}catch(e){storageReady=false;lastStorageError=e.message;supabaseMigrationPending=false;console.error('Google DATA storage unavailable:',e.message)}if(supabaseMigrationPending){await saveAcademyData(data);supabaseActive=true;supabaseMigrationPending=false;storageReady=true;lastStorageError='';console.log('Academy data migrated from legacy Google DATA storage to Supabase.');}}
...
function persistExamStorage(which=['exams','results','attempts']){
  // Legacy fallback only. Live Supabase exam routes use targeted row writes.
  if(supabaseActive)return Promise.resolve({ok:true,targeted:true});
...
function sess(req){try{const token=jwt.verify(req.cookies.kayan_session,SESSION_SECRET);if(Number(token?.sessionEpoch||0)!==Number(data.settings?.sessionEpoch||0))return null;return token}catch{return null}}
...
function encryptDevToken(value){const iv=crypto.randomBytes(12),cipher=crypto.createCipheriv('aes-256-gcm',devTokenKey(),iv);const enc=Buffer.concat([cipher.update(String(value||''),'utf8'),cipher.final()]);return [iv.toString('base64url'),cipher.getAuthTag().toString('base64url'),enc.toString('base64url')].join('.')}
function decryptDevToken(value){try{const [iv,tag,enc]=String(value||'').split('.');if(!iv||!tag||!enc)return '';const decipher=crypto.createDecipheriv('aes-256-gcm',devTokenKey(),Buffer.from(iv,'base64url'));decipher.setAuthTag(Buffer.from(tag,'base64url'));return Buffer.concat([decipher.update(Buffer.from(enc,'base64url')),decipher.final()]).toString('utf8')}catch{return ''}}
async function discordGuilds(accessToken){const r=await fetch('https://discord.com/api/v10/users/@me/guilds',{headers:{Authorization:'Bearer '+accessToken},signal:AbortSignal.timeout(10000)});if(!r.ok)throw Error('Discord guild lookup failed');return await r.json()}
async function refreshDevOAuth(uid,entry){const refresh=decryptDevToken(entry?.refreshToken);if(!refresh)return '';const body=new URLSearchParams({client_id:CLIENT_ID,client_secret:CLIENT_SECRET,grant_type:'refresh_token',refresh_token:refresh});const r=await fetch('https://discord.com/api/v10/oauth2/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body,signal:AbortSignal.timeout(10000)});if(!r.ok)return '';const tk=await r.json();entry.accessToken=encryptDevToken(tk.access_token);entry.refreshToken=encryptDevToken(tk.refresh_token||refresh);entry.expiresAt=Date.now()+Number(tk.expires_in||604800)*1000;data.devStoreTokens[uid]=entry;await save().catch(()=>{});return String(tk.access_token||'')}
async function checkDevStoreMembership(uid){const entry=data.devStoreTokens?.[uid];if(!entry)return {joined:false,oauthRequired:true};let access=Date.now()<Number(entry.expiresAt||0)-60000?decryptDevToken(entry.accessToken):'';if(!access)access=await refreshDevOAuth(uid,entry);if(!access)return {joined:Boolean(entry.joined===true),oauthRequired:true,checkError:'DISCORD_OAUTH_REQUIRED'};try{const guilds=await discordGuilds(access);const joined=Array.isArray(guilds)&&guilds.some(g=>String(g?.id)===DEV_STORE_GUILD_ID);const changed=Boolean(entry.joined)!==joined;entry.lastCheckedAt=Date.now();entry.joined=joined;if(joined)entry.joinedAt=entry.joinedAt||new Date().toISOString();else entry.joinedAt=null;data.devStoreTokens[uid]=entry;if(changed)await save().catch(()=>{});return {joined,oauthRequired:false,verifiedAt:joined?(entry.joinedAt||entry.lastCheckedAt):null}}catch(e){return {joined:Boolean(entry.joined===true),oauthRequired:false,checkError:String(e?.message||e)}}}
async function current(req){const x=sess(req);if(!x)return{x:null,police:null,role:'citizen',admin:false,permissions:[]};const a=admin(x.id),isAdmin=isSuperAdmin(x.id)||Boolean(a?.enabled)||isBootstrapAdmin(x.id);try{const rows=await withTimeout(police(),4000),p=rows.find(r=>id(r.discordId)===id(x.id))||null;return{x,police:p,role:role(p),admin:isAdmin,isSuperAdmin:isSuperAdmin(x.id),permissions:perms(x.id),sheet:true}}catch(e){return{x,police:null,role:'unknown',admin:isAdmin,isSuperAdmin:isSuperAdmin(x.id),permissions:perms(x.id),sheet:false,error:e.message}}}
...
// Generic mutations may roll back. Live exams are different: answers must never
// disappear just because one persistence request times out.
async function saved(fn,req,res){const snapshot=structuredClone(data);try{return await fn()}catch(e){data=snapshot;console.error('API mutation failed:',e);if(res.headersSent)return;const clientErrors=new Set(['INVALID_TIME_RANGE','EXAM_ALLOWED_USERS_REQUIRED','CHOICE_CORRECT_OPTION_REQUIRED','YESNO_CORRECT_ANSWER_REQUIRED','INVALID_DISCORD_ID','INVALID_IMAGE','IMAGE_FORMAT_REQUIRED','IMAGE_TOO_LARGE']);if(clientErrors.has(String(e?.message||'')))return res.status(400).json({error:String(e.message),message:String(e.message),retryable:false});return res.status(503).json({error:'STORAGE_ERROR',message:'تعذر حفظ البيانات حاليًا. تم التراجع عن العملية ولم تُحفظ بيانات ناقصة.',retryable:true})}}
async function persistAttemptSafe(attempt){
  if(!supabaseActive) return persistExamStorage(['attempts']);
  try{
    const persisted=await saveExamAttempt(attempt);
    const canonical=persitedAttemptShape(persisted,attempt);
    return {ok:true,attempt:canonical};
  }catch(error){
    console.error('Live exam autosave failed:',error?.message||error);
    return {ok:false,error};
  }
}
function persitedAttemptShape(persisted,fallback){
  const legacy=persisted?.legacy_data&&typeof persisted.legacy_data==='object'?persisted.legacy_data:{};
  return {...fallback,...legacy,id:String(legacy.id||persisted?.legacy_id||fallback.id)};
}
...
function cleanExam(e){const questions=Array.isArray(e?.questions)?e.questions.map(cleanQuestion):[],accessType=EXAM_ACCESS.includes(e?.accessType)?e?.accessType:'police';const accessToken=accessType==='link'?(String(e?.accessToken||'').trim()||crypto.randomBytes(18).toString('base64url')):'';const allowedDiscordIds=accessType==='specific'?(Array.isArray(e?.allowedDiscordIds)?e.allowedDiscordIds.map(v=>String(v).trim()).filter(Boolean):[]):[];const rawSections=Array.isArray(e?.sections)?e.sections.map((x,i)=>({id:String(x?.id||('section-'+Date.now()+'-'+i)).trim(),title:String(x?.title||('القسم '+(i+1))).trim().slice(0,200),description:String(x?.description||'').trim().slice(0,2000),gateQuestionId:String(x?.gateQuestionId||'').trim(),allowedAnswers:Array.isArray(x?.allowedAnswers)?x.allowedAnswers.map(v=>String(v).trim()).filter(Boolean).slice(0,50):[],failMessage:String(x?.failMessage||'').trim().slice(0,1000),nextSectionId:String(x?.nextSectionId||'').trim()})):[];const sections=rawSections.map((s,i)=>{if(i===0)return{...s,gateQuestionId:'',allowedAnswers:[]};const previousId=String(rawSections[i-1]?.id||'');const gateId=String(s.gateQuestionId||'');const gateQuestion=questions.find(q=>String(q.id)===gateId);const validGate=Boolean(gateQuestion&&String(gateQuestion.sectionId||'')===previousId);return{...s,gateQuestionId:validGate?gateId:'',allowedAnswers:validGate?s.allowedAnswers:[]};});return{id:String(e?.id||('exam-'+Date.now())),title:String(e?.title||'اختبار جديد'),description:String(e?.description||''),stage:String(e?.stage||'عام'),accessType,accessToken,allowedDiscordIds,passingScore:Math.max(1,Math.min(100,Number(e?.passingScore||60))),durationMinutes:Math.max(1,Number(e?.durationMinutes||30)),attemptsAllowed:Math.max(1,Number(e?.attemptsAllowed||1)),startAt:validDate(e?.startAt)?iso(e.startAt):null,endAt:validDate(e?.endAt)?iso(e.endAt):null,active:e?.active!==false,resultPublished:e?.resultPublished===true,resultAnswersPublished:e?.resultAnswersPublished===true,createdAt:e?.createdAt||new Date().toISOString(),bannerUrl:String(e?.bannerUrl||'').trim(),sections,questions}}function shuffleQuestions(list){const a=[...(Array.isArray(list)?list:[])];for(let i=a.length-1;i>0;i--){const j=crypto.randomInt(i+1);[a[i],a[j]]=[a[j],a[i]]}return a}
function orderedExam(e,attempt){const ids=Array.isArray(attempt?.questionOrder)?attempt.questionOrder.map(String):[];const by=new Map((e.questions||[]).map(q=>[String(q.id),q]));const ordered=ids.length?ids.map(id=>by.get(id)).filter(Boolean):shuffleExamQuestions(e);return publicExam({...e,questions:ordered})}
function shuffleExamQuestions(e){const qs=Array.isArray(e?.questions)?e.questions:[];const sections=Array.isArray(e?.sections)?e.sections:[];if(!sections.length)return shuffleQuestions(qs);const used=new Set();const ordered=[];for(const section of sections){const inSection=qs.filter(q=>String(q.sectionId||'')===String(section.id));for(const q of shuffleQuestions(inSection)){ordered.push(q);used.add(String(q.id))}}for(const q of shuffleQuestions(qs.filter(q=>!used.has(String(q.id)))))ordered.push(q);return ordered}
function scoreAttempt(e,answers){let earned=0,total=0;for(const q of e.questions||[]){const p=Math.max(0,Number(q.points??1));if(p>0)total+=p;if((q.type==='choice'||q.type==='yesno')&&String(answers?.[q.id]??answers?.[q.legacyId]??answers?.[q.legacy_id]??'')===String(q.correct??''))earned+=p}return total?Math.round(earned/total*100):0}
function recoverSubmittedExamResults(){
  ...
  for(const a of attempts){
    ...
    const review=(exam.questions||[]).map(q=>({id:q.id,text:q.text,type:q.type,answer:answers?.[q.id]??'',correct:q.correct??'',points:Number(q.points??1)}));
    if(!result){
...
function publicExam(e){const questions=(Array.isArray(e?.questions)?e.questions:[]).map(cleanQuestion).filter(q=>q.text).map(q=>({id:q.id,text:q.text,type:q.type,options:q.options||[],required:q.required!==false,points:q.points,sectionId:String(q.sectionId||''),imageUrl:String(q.imageUrl||'').trim()||undefined}));return{...e,state:timeState(e?.startAt,e?.endAt),accessToken:undefined,allowedDiscordIds:undefined,questions}}
...
app.get('/api/health',async(_q,res)=>{
  ...
  res.json({
    ...
    adminConfigured:data.admins.some(a=>a.enabled)||BOOTSTRAP_ADMINS.size>0,
    academyDataSheetId:DATA_SHEET_ID,
    academyDataSheetName:DATA_SHEET
  })
});app.get('/api/me',async(req,res)=>{const c=await current(req);if(!c.x)return res.json({authenticated:false,role:'citizen',permissions:{isCitizen:true,isOfficer:false,isAdmin:false,adminPermissions:[],isSuperAdmin:false,canViewEvaluations:false}});if(!c.sheet)return res.json({authenticated:true,identityPending:true,discord:c.x,police:null,role:'unknown',permissions:{isCitizen:false,isOfficer:false,isAdmin:c.admin,isSuperAdmin:c.isSuperAdmin,adminPermissions:c.permissions,canViewEvaluations:false}});const officer=Boolean(c.police);let dispatchAccess=null;try{dispatchAccess=await getAccess(c.x.id)}catch{};res.json({authenticated:true,identityPending:false,discord:c.x,police:c.police?{name:c.police.name,rank:c.police.rank,code:c.police.code,badge:c.police.badge,status:c.police.status,responsibility:c.police.responsibility,department:c.police.department||c.police.responsibility||'',leave:c.police.leave,discordId:c.police.discordId,image:data.memberImages?.[c.police.discordId]||''}:null,role:officer?c.role:'citizen',permissions:{isCitizen:!officer,isOfficer:officer,isAdmin:c.admin,isSuperAdmin:c.isSuperAdmin,adminPermissions:c.permissions,canViewEvaluations:c.admin&&c.permissions.includes('view_evaluations'),dispatchPermissions:Array.isArray(dispatchAccess?.permissions)?dispatchAccess.permissions:[],dispatchAccessEnabled:Boolean(dispatchAccess?.enabled)}})});
...
app.get('/api/public/exams',async(req,res)=>{const c=await current(req);if(!c.x)return res.status(401).json({error:'UNAUTHENTICATED'});await finalizeExpiredAttempts();const token=String(req.query.token||''),t=now();const exams=data.exams.filter(e=>{const state=timeState(e.startAt,e.endAt);return e.active===true&&state==='open'&&examAllowed(e,c,token)}).map(publicExam);res.json({exams,serverTime:new Date(t).toISOString()})});
...
app.get('/api/police-codes',async(req,res)=>{try{const c=await current(req);if(!c.x)return res.status(401).json({error:'UNAUTHENTICATED'});if(!supabaseActive)return res.json({items:[]});const {data,error}=await (await import('./supabase.mjs')).requireSupabase().from('police_codes').select('*').eq('active',true).order('category_position').order('position');if(error)throw error;const groups=[];const by=new Map();for(const x of data||[]){let g=by.get(x.category_id);if(!g){g={id:x.category_id,title:x.category_title,icon:x.category_icon,tone:x.category_tone,items:[]};by.set(x.category_id,g);groups.push(g)}g.items.push({code:x.code,description:x.description,position:x.position})}res.json({items:groups})}catch(e){console.error('Police codes API failed:',e);res.status(500).json({error:'STORAGE_ERROR'})}});
...
app.post('/api/exams/:id/start',(req,res)=>saved(async()=>{const c=await current(req);if(!c.x)return res.status(401).json({error:'UNAUTHENTICATED'});await finalizeExpiredAttempts();const e=data.exams.find(x=>x.id===req.params.id&&x.active);if(!e)return res.status(404).json({error:'EXAM_NOT_FOUND'});const token=String(req.body?.accessToken||req.query.token||'');if(!examAllowed(e,c,token))return res.status(403).json({error:'EXAM_ACCESS_DENIED'});const uid=String(c.x.id),allowed=Math.max(1,Number(e.attemptsAllowed||1)),attempts=data.examAttempts.filter(a=>a.examId===e.id&&a.userId===uid);let attempt=attempts.find(a=>!a.submittedAt);if(attempt){const isAdminResume=attempt.status==='paused'&&Boolean(attempt.resumeAt);if(attempt.resumeAt&&now()<new Date(attempt.resumeAt).getTime())return res.status(409).json({error:'EXAM_RESUME_NOT_STARTED'});if(attempt.resumeUntil&&now()>=new Date(attempt.resumeUntil).getTime()){attempt.expiresAt=new Date(attempt.resumeUntil).toISOString();await finalizeExpiredAttempts();return res.status(409).json({error:'EXAM_TIME_EXPIRED'})}if(!isAdminResume){const state=timeState(e.startAt,e.endAt);if(state==='upcoming')return res.status(409).json({error:'EXAM_NOT_STARTED'});if(state==='ended')return res.status(409).json({error:'EXAM_ENDED'})}return res.json({ok:true,attempt,exam:orderedExam(e,attempt)})}const state=timeState(e.startAt,e.endAt);if(state==='upcoming')return res.status(409).json({error:'EXAM_NOT_STARTED'});if(state==='ended')return res.status(409).json({error:'EXAM_ENDED'});if(data.examResults.some(r=>r.examId===e.id&&r.userId===uid))return res.status(409).json({error:'EXAM_ALREADY_SUBMITTED'});if(attempts.length>=allowed)return res.status(409).json({error:'EXAM_ATTEMPTS_EXHAUSTED'});const started=new Date(),duration=Math.max(1,Number(e.durationMinutes||30))*60000,endWindow=validDate(e.endAt)?new Date(e.endAt).getTime():Infinity,expiresAt=new Date(Math.min(started.getTime()+duration,endWindow)).toISOString(),ordered=shuffleExamQuestions(e);attempt={id:'attempt-'+Date.now()+'-'+crypto.randomBytes(3).toString('hex'),examId:e.id,userId:uid,name:c.police?.name||c.x.global_name||c.x.username||'متقدم',startedAt:started.toISOString(),expiresAt,submittedAt:null,status:'active',accessToken:e.accessType==='link'?token:'',answers:{},questionOrder:ordered.map(q=>String(q.id)),examSnapshot:structuredClone(e),activeStartedAt:started.toISOString(),activeDurationSeconds:0,lastSavedAt:started.toISOString()};const persisted=await persistAttemptSafe(attempt);if(!persisted.ok)return res.status(503).json({error:'EXAM_SAVE_PENDING',message:'تعذر تأكيد حفظ محاولة الامتحان الآن. لم يتم فتح محاولة غير مؤكدة. أعد المحاولة بعد لحظات.',retryable:true});attempt=persisted.attempt||attempt;data.examAttempts=data.examAttempts.filter(x=>!(String(x.examId)===String(e.id)&&String(x.userId)===uid&&!x.submittedAt));data.examAttempts.push(attempt);res.json({ok:true,attempt,exam:orderedExam(e,attempt)})},req,res));function cleanupActivityLogs(retentionDays=7){
 const cutoff=Date.now()-Math.max(1,Number(retentionDays||7))*24*60*60*1000;
 const keep=x=>{const t=new Date(x?.at||0).getTime();return !Number.isFinite(t)||t>=cutoff};
 const auditBefore=Array.isArray(data.audit)?data.audit.length:0;
 const loginBefore=Array.isArray(data.loginLogs)?data.loginLogs.length:0;
 data.audit=(data.audit||[]).filter(keep);
 data.loginLogs=(data.loginLogs||[]).filter(keep);
 return {auditDeleted:auditBefore-data.audit.length,loginDeleted:loginBefore-data.loginLogs.length};
}
let lastActivityCleanupDay='';
function scheduleActivityLogCleanup(){
 const tick=()=>{
   const nowCairo=new Date(new Date().toLocaleString('en-US',{timeZone:'Africa/Cairo'}));
   const day=nowCairo.toISOString().slice(0,10);
   if(nowCairo.getHours()===0&&nowCairo.getMinutes()<10&&lastActivityCleanupDay!==day){
     lastActivityCleanupDay=day;
     const removed=cleanupActivityLogs(7);
     if(removed.auditDeleted||removed.loginDeleted)save().catch(e=>console.error('Automatic activity log cleanup failed:',e.message));
   }
 };
 tick();
 setInterval(tick,60*1000).unref?.();
}
app.post('/api/exams/:id/submit',(req,res)=>saved(async()=>{
  const c=await current(req);if(!c.x)return res.status(401).json({error:'UNAUTHENTICATED'});
  const uid=String(c.x.id),examId=String(req.params.id);
  const liveExam=data.exams.find(x=>String(x.id)===examId);
  // Keep the same attempt discoverable even after a timeout/retry. A duplicate
  // submit is idempotent: the student gets the original result instead of a
  // confusing EXAM_NOT_STARTED response.
  let attempt=data.examAttempts.find(x=>String(x.examId)===examId&&String(x.userId)===uid&&!x.submittedAt);
  const submittedAttempt=data.examAttempts.find(x=>String(x.examId)===examId&&String(x.userId)===uid&&x.submittedAt);
  const existing=data.examResults.find(x=>String(x.examId)===examId&&String(x.userId)===uid);
  if(existing)return res.json({ok:true,result:existing,autoSubmitted:Boolean(existing.autoSubmitted),submitted:true,idempotent:true});
  if(!attempt&&submittedAttempt)attempt=submittedAttempt;
  if(!attempt)return res.status(409).json({error:'EXAM_NOT_STARTED'});
  const e=liveExam||attempt.examSnapshot;if(!e)return res.status(404).json({error:'EXAM_NOT_FOUND'});
  const token=String(req.body?.accessToken||req.query.token||attempt?.accessToken||'');
  if(!examAllowed(e,c,token))return res.status(403).json({error:'EXAM_ACCESS_DENIED'});
  const examForAttempt=attempt.examSnapshot||e;
  ...
  const currentSectionId=String(req.body?.sectionId||'');
  const sections=Array.isArray(examForAttempt.sections)?examForAttempt.sections:[];
  const currentIndex=currentSectionId?sections.findIndex(sec=>String(sec.id)===currentSectionId):-1;
  const currentSection=currentIndex>=0?sections[currentIndex]:null;
  const explicitNextId=String(currentSection?.nextSectionId||'');
  const nextIndex=explicitNextId?sections.findIndex(sec=>String(sec.id)===explicitNextId):currentIndex+1;
  const nextSection=nextIndex>=0&&nextIndex<sections.length?sections[nextIndex]:null;
  const gateSection=nextSection;
  const gateFailed=Boolean(gateSection&&gateSection.gateQuestionId&&Array.isArray(gateSection.allowedAnswers)&&gateSection.allowedAnswers.length&& !gateSection.allowedAnswers.map(String).map(v=>v.trim()).filter(Boolean).includes(String(answers[gateSection.gateQuestionId]??'').trim()));
  ...
  const activeStart=attempt.activeStartedAt?new Date(attempt.activeStartedAt).getTime():new Date(attempt.startedAt).getTime();
  const activeEnd=expired&&attempt.expiresAt?Math.min(new Date(submittedAt).getTime(),new Date(attempt.expiresAt).getTime()):new Date(submittedAt).getTime();
  const activeDurationSeconds=attempt.activeDurationSeconds?Number(attempt.activeDurationSeconds):activeBase+Math.max(0,Math.round((activeEnd-activeStart)/1000));
  ...
  Object.assign(attempt,durableAttempt);
  if(!data.examResults.some(x=>String(x.id)===String(result.id)||String(x.attemptId||'')===String(attempt.id)))data.examResults.unshift(result);
  for(const other of data.examAttempts||[])if(other!==attempt&&String(other.examId)===String(e.id)&&String(other.userId)===uid&&!other.submittedAt){other.submittedAt=submittedAt;other.status='superseded';other.superseded=true;}
  res.json({ok:true,result,autoSubmitted:Boolean(expired),submitted:true,reviewPending:!e.resultPublished});
},req,res));
// Live autosave is isolated from generic mutations: never clone the full
// academy dataset for a single answer snapshot.
app.put('/api/exams/:id/attempt',async(req,res)=>{try{const c=await current(req);if(!c.x)return res.status(401).json({error:'UNAUTHENTICATED'});const e=data.exams.find(x=>x.id===req.params.id&&x.active);if(!e)return res.status(404).json({error:'EXAM_NOT_FOUND'});const uid=String(c.x.id),attempt=data.examAttempts.find(x=>x.examId===e.id&&x.userId===uid&&!x.submittedAt);if(!attempt)return res.status(409).json({error:'EXAM_NOT_STARTED'});const examForAttempt=attempt.examSnapshot||e;const token=String(req.body?.accessToken||req.query.token||attempt.accessToken||'');if(!examAllowed(e,c,token))return res.status(403).json({error:'EXAM_ACCESS_DENIED'});if(now()>=new Date(attempt.expiresAt).getTime())return res.status(409).json({error:'EXAM_TIME_EXPIRED'});const answers=req.body?.answers&&typeof req.body.answers==='object'?req.body.answers:{};attempt.answers={...(attempt.answers||{}),...Object.fromEntries(Object.entries(answers).map(([k,v])=>[k,String(v??'').slice(0,5000)]))};attempt.lastSavedAt=new Date().toISOString();const persisted=await persistAttemptSafe(attempt);if(!persisted.ok)return res.status(503).json({error:'EXAM_AUTOSAVE_PENDING',message:'تم الاحتفاظ بإجابتك مؤقتًا لكن تعذر تأكيد حفظها على الخادم. أعد المحاولة تلقائيًا.',retryable:true,lastSavedAt:attempt.lastSavedAt});res.json({ok:true,lastSavedAt:attempt.lastSavedAt})}catch(e){console.error('Exam autosave route failed:',e);if(!res.headersSent)res.status(503).json({error:'EXAM_AUTOSAVE_PENDING',retryable:true})}});
...
app.post('/api/admin/badges',async(req,res)=>{try{const c=await requireAdmin(req,res,'manage_members');if(!c)return;const name=String(req.body?.name||'').trim().slice(0,80);const description=String(req.body?.description||'').trim().slice(0,240);const icon=String(req.body?.icon||'🏅').trim().slice(0,16);const iconUrl=String(req.body?.iconUrl||'').trim().slice(0,1000);if(name.length<2)return res.status(400).json({error:'INVALID_BADGE'});if(iconUrl){try{const u=new URL(iconUrl);if(!['http:','https:'].includes(u.protocol))return res.status(400).json({error:'INVALID_BADGE_IMAGE'})}catch{return res.status(400).json({error:'INVALID_BADGE_IMAGE'})}}const badge={id:'badge-'+Date.now()+'-'+crypto.randomBytes(3).toString('hex'),name,description,icon,iconUrl,createdAt:new Date().toISOString(),createdBy:String(c.x.id)};data.badges=Array.isArray(data.badges)?data.badges:[];data.badges.unshift(badge);audit(c,'CREATE_BADGE',badge.id,name);await save();res.json({ok:true,badge})}catch(e){console.error(e);res.status(503).json({error:'STORAGE_ERROR'})}});
app.patch('/api/admin/badges/:id',async(req,res)=>{try{const c=await requireAdmin(req,res,'manage_members');if(!c)return;const badge=(data.badges||[]).find(b=>String(b.id)===String(req.params.id));if(!badge)return res.status(404).json({error:'BADGE_NOT_FOUND'});if(req.body?.name!==undefined)badge.name=String(req.body.name||'').trim().slice(0,80);if(req.body?.description!==undefined)badge.description=String(req.body.description||'').trim().slice(0,240);if(req.body?.icon!==undefined)badge.icon=String(req.body.icon||'🏅').trim().slice(0,16);if(req.body?.iconUrl!==undefined){const url=String(req.body.iconUrl||'').trim().slice(0,1000);if(url){try{const u=new URL(url);if(!['http:','https:'].includes(u.protocol))return res.status(400).json({error:'INVALID_BADGE_IMAGE'})}catch{return res.status(400).json({error:'INVALID_BADGE_IMAGE'})}}badge.iconUrl=url}if(badge.name.length<2)return res.status(400).json({error:'INVALID_BADGE'});badge.updatedAt=new Date().toISOString();audit(c,'UPDATE_BADGE',badge.id,badge.name);await saveDurable();res.json({ok:true,badge})}catch(e){res.status(503).json({error:'STORAGE_ERROR'})}});
app.delete('/api/admin/badges/:id',async(req,res)=>{try{const c=await requireAdmin(req,res,'manage_members');if(!c)return;const before=data.badges?.length||0;data.badges=(data.badges||[]).filter(b=>String(b.id)!==String(req.params.id));if(before===data.badges.length)return res.status(404).json({error:'BADGE_NOT_FOUND'});for(const uid of Object.keys(data.memberBadges||{}))data.memberBadges[uid]=(data.memberBadges[uid]||[]).filter(x=>String(x)!==String(req.params.id));audit(c,'DELETE_BADGE',req.params.id);await saveDurable();res.json({ok:true})}catch(e){res.status(503).json({error:'STORAGE_ERROR'})}});
app.post('/api/admin/member-badges',async(req,res)=>{try{const c=await requireAdmin(req,res,'manage_members');if(!c)return;const discordId=String(req.body?.discordId||'').trim(),badgeId=String(req.body?.badgeId||'').trim();if(!discordId||!badgeId)return res.status(400).json({error:'BADGE_ASSIGNMENT_REQUIRED'});if(!(data.badges||[]).some(b=>String(b.id)===badgeId))return res.status(404).json({error:'BADGE_NOT_FOUND'});data.memberBadges=data.memberBadges&&typeof data.memberBadges==='object'?data.memberBadges:{};const list=Array.isArray(data.memberBadges[discordId])?data.memberBadges[discordId]:[];if(!list.includes(badgeId))list.unshift(badgeId);data.memberBadges[discordId]=list;audit(c,'ASSIGN_BADGE',discordId,badgeId);await saveDurable();res.json({ok:true,badgeIds:list})}catch(e){res.status(503).json({error:'STORAGE_ERROR'})}});
app.delete('/api/admin/member-badges',async(req,res)=>{try{const c=await requireAdmin(req,res,'manage_members');if(!c)return;const discordId=String(req.body?.discordId||'').trim(),badgeId=String(req.body?.badgeId||'').trim();data.memberBadges=data.memberBadges&&typeof data.memberBadges==='object'?data.memberBadges:{};data.memberBadges[discordId]=(data.memberBadges[discordId]||[]).filter(x=>String(x)!==badgeId);audit(c,'REMOVE_BADGE',discordId,badgeId);await saveDurable();res.json({ok:true,badgeIds:data.memberBadges[discordId]})}catch(e){res.status(503).json({error:'STORAGE_ERROR'})}});
...
app.get('/api/admin/state',async(req,res)=>{const c=await requireAdmin(req,res);if(!c)return;const expired=expireBatches();if(expired.length)await save().catch(e=>console.error('Auto-close save failed:',e.message));const batches=(data.batches||[]).map(b=>({...b,state:batchState(b)}));res.json({settings:data.settings,applicationQuestions:data.applicationQuestions,questionBank:data.questionBank,batches,applications:data.applications,exams:data.exams,examResults:data.examResults,examAttempts:data.examAttempts,evaluations:data.evaluations,hierarchy:data.hierarchy,admins:[...data.admins,...(SUPER_ADMIN_ID&&!data.admins.some(a=>id(a.discordId)===SUPER_ADMIN_ID)?[{discordId:SUPER_ADMIN_ID,name:'Super Admin (ENV)',permissions:ALL,enabled:true,envProtected:true}]:[])],permissions:PERMISSIONS,badges:Array.isArray(data.badges)?data.badges:[],memberBadges:data.memberBadges&&typeof data.memberBadges==='object'?data.memberBadges:{},members:(await police()).map(member)})});
...
app.get('/api/admin/activity-state',async(req,res)=>{
 ...
 res.json({audit,loginLogs,departments,loginCount:Number(data.loginCount||0)});
});
app.post('/api/admin/activity-logs/clear-old',async(req,res)=>{
 const c=await requireAdmin(req,res,'view_activity_logs');if(!c)return;
 try{
   const removed=cleanupActivityLogs(3);
   await saveDurable();
   res.json({ok:true,...removed,retentionDays:3});
 }catch(e){console.error('Manual activity log cleanup failed:',e);res.status(503).json({error:'ACTIVITY_LOG_CLEANUP_FAILED',retryable:true})}
});
...
app.post('/api/admin/exams',(req,res)=>saved(async()=>{const c=await requireAdmin(req,res,'manage_exams');if(!c)return;const e=cleanExam(req.body);validateExam(e);data.exams.unshift(e);audit(c,'CREATE_EXAM',e.id,`${e.title} · ${e.accessType}`);if(supabaseActive)await saveExam(e);else await persistExamStorage(['exams']);res.json({ok:true,exam:e})},req,res));
app.put('/api/admin/exams/:id',(req,res)=>saved(async()=>{const c=await requireAdmin(req,res,'manage_exams');if(!c)return;const old=data.exams.find(x=>x.id===req.params.id);if(!old)return res.status(404).json({error:'EXAM_NOT_FOUND'});const e=cleanExam({...old,...req.body,id:old.id,createdAt:old.createdAt,accessToken:req.body?.accessToken??old.accessToken});validateExam(e);Object.assign(old,e);audit(c,'UPDATE_EXAM',old.id);if(supabaseActive)await saveExam(old);else await persistExamStorage(['exams']);res.json({ok:true,exam:old})},req,res));
app.patch('/api/admin/exams/:id/active',(req,res)=>saved(async()=>{const c=await requireAdmin(req,res,'manage_exams');if(!c)return;const e=data.exams.find(x=>String(x.id)===String(req.params.id));if(!e)return res.status(404).json({error:'EXAM_NOT_FOUND'});e.active=req.body?.active===true;e.status=e.active?'open':'closed';audit(c,e.active?'ACTIVATE_EXAM':'DEACTIVATE_EXAM',e.id);if(supabaseActive)await saveExam(e);else await persistExamStorage(['exams']);res.json({ok:true,exam:e})},req,res));
app.delete('/api/admin/exams/:id',(req,res)=>saved(async()=>{const c=await requireAdmin(req,res,'manage_exams');if(!c)return;const examId=String(req.params.id);const exam=data.exams.find(x=>String(x.id)===examId);if(!exam)return res.status(404).json({error:'EXAM_NOT_FOUND'});const results=data.examResults.filter(r=>String(r.examId)===examId).length,attempts=data.examAttempts.filter(a=>String(a.examId)===examId).length;if(supabaseActive){const deleted=await deleteExam(examId);if(!deleted)return res.status(404).json({error:'EXAM_NOT_FOUND'});}else{await persistExamStorage(['exams','results','attempts']);}data.exams=data.exams.filter(x=>String(x.id)!==examId);data.examResults=data.examResults.filter(r=>String(r.examId)!==examId);data.examAttempts=data.examAttempts.filter(a=>String(a.examId)!==examId);audit(c,'DELETE_EXAM',examId,`title=${String(exam.title||'اختبار')};removedResults=${results};removedAttempts=${attempts}`);res.json({ok:true,removedResults:results,removedAttempts:attempts})},req,res));
...
app.get('/auth/discord',(_q,res)=>{if(!CLIENT_ID||!CLIENT_SECRET)return res.status(503).send('Discord OAuth is not configured yet.');const p=new URLSearchParams({client_id:CLIENT_ID,response_type:'code',redirect_uri:REDIRECT,scope:'identify guilds'});if(String(_q.query?.devstore||'')==='1')p.set('prompt','consent');res.redirect(`https://discord.com/oauth2/authorize?${p}`)});
app.get('/auth/discord/callback',async(req,res)=>{try{const code=String(req.query.code||'');if(!code)throw Error('Missing OAuth code');const body=new URLSearchParams({client_id:CLIENT_ID,client_secret:CLIENT_SECRET,grant_type:'authorization_code',code,redirect_uri:REDIRECT});const tr=await fetch('https://discord.com/api/v10/oauth2/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body,signal:AbortSignal.timeout(10000)});if(!tr.ok)throw Error('Discord token exchange failed');const tk=await tr.json();const mr=await fetch('https://discord.com/api/v10/users/@me',{headers:{Authorization:`Bearer ${tk.access_token}`},signal:AbortSignal.timeout(10000)});if(!mr.ok)throw Error('Discord identity lookup failed');const me=await mr.json();const devStoreMode=String(req.query.devstore||'')==='1';data.devStoreTokens= data.devStoreTokens&&typeof data.devStoreTokens==='object'?data.devStoreTokens:{};const previousEntry=data.devStoreTokens[String(me.id)]||{};const devEntry={accessToken:encryptDevToken(tk.access_token),refreshToken:encryptDevToken(tk.refresh_token||''),expiresAt:Date.now()+Number(tk.expires_in||604800)*1000,lastCheckedAt:previousEntry.lastCheckedAt||0,joined:previousEntry.joined===true,joinedAt:previousEntry.joinedAt||null,lastPromptAt:previousEntry.lastPromptAt||0};data.devStoreTokens[String(me.id)]={...previousEntry,...devEntry};try{const guilds=await discordGuilds(tk.access_token);const joined=Array.isArray(guilds)&&guilds.some(g=>String(g?.id)===DEV_STORE_GUILD_ID);data.devStoreTokens[String(me.id)].joined=joined;data.devStoreTokens[String(me.id)].lastCheckedAt=Date.now();if(joined)data.devStoreTokens[String(me.id)].joinedAt=data.devStoreTokens[String(me.id)].joinedAt||new Date().toISOString();else{data.devStoreTokens[String(me.id)].joinedAt=null;data.devStoreTokens[String(me.id)].lastPromptAt=0}}catch(e){console.error('Dev store membership check failed:',e.message)}const token=jwt.sign({id:me.id,username:me.username,global_name:me.global_name,avatar:me.avatar,sessionEpoch:Number(data.settings?.sessionEpoch||0)},SESSION_SECRET,{expiresIn:'7d'});res.cookie('kayan_session',token,{httpOnly:true,sameSite:'lax',secure:process.env.NODE_ENV==='production',maxAge:604800000,path:'/'});data.loginCount=Math.max(0,Number(data.loginCount||0))+1;data.loginLogs.unshift({id:`login-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`,at:new Date().toISOString(),discordId:String(me.id),name:me.global_name||me.username||'غير معروف',username:me.username||'',avatar:me.avatar||'',avatarUrl:me.avatar?`https://cdn.discordapp.com/avatars/${me.id}/${me.avatar}.png?size=128`:''});data.loginLogs=data.loginLogs.slice(0,2000);try{await save()}catch(e){console.error('login log save failed:',e.message)}if(devStoreMode){const joined=data.devStoreTokens?.[String(me.id)]?.joined===true;res.redirect(joined?(FRONTEND||'/'):(DEV_STORE_INVITE))}else res.redirect(FRONTEND||'/')}catch(e){res.status(500).send(`Discord login failed: ${e.message}`)}});
app.get('/api/dev-store/config',async(_req,res)=>{const settings=data.settings?.devStore||{};let iconUrl='';let guildName='متجر التطوير';try{const code=String(settings.inviteUrl||DEV_STORE_INVITE).split('/').pop();const ir=await fetch('https://discord.com/api/v10/invites/'+encodeURIComponent(code)+'?with_counts=true',{signal:AbortSignal.timeout(8000)});if(ir.ok){const x=await ir.json();guildName=x?.guild?.name||guildName;if(x?.guild?.id&&x?.guild?.icon)iconUrl='https://cdn.discordapp.com/icons/'+x.guild.id+'/'+x.guild.icon+'.webp?size=256'}}catch{}res.json({guildId:String(settings.guildId||DEV_STORE_GUILD_ID),inviteUrl:String(settings.inviteUrl||DEV_STORE_INVITE),guildName,iconUrl,promptIntervalHours:Math.max(1,Number(settings.promptIntervalHours||3))})});
app.get('/api/dev-store/status',async(req,res)=>{const c=await current(req);if(!c.x)return res.status(401).json({authenticated:false});const uid=String(c.x.id);const settings=data.settings?.devStore||{};const entry=data.devStoreTokens?.[uid]||{};if(entry.joined===true)return res.json({joined:true,show:false,oauthRequired:false,verifiedAt:entry.joinedAt||entry.lastCheckedAt||null});const check=await checkDevStoreMembership(uid);if(check.joined){entry.lastPromptAt=0;data.devStoreTokens[uid]=entry;return res.json({joined:true,show:false,oauthRequired:false,verifiedAt:entry.joinedAt||entry.lastCheckedAt||null})}const nowMs=Date.now(),interval=Math.max(1,Number(settings.promptIntervalHours||3))*3600000,resetAt=Number(settings.promptResetAt||0);const due=Number(entry.lastPromptAt||0)<resetAt||nowMs-Number(entry.lastPromptAt||0)>=interval;const show=due;if(show){entry.lastPromptAt=nowMs;data.devStoreTokens[uid]=entry;await save().catch(()=>{})}res.json({joined:false,show,oauthRequired:Boolean(check.oauthRequired),inviteUrl:String(settings.inviteUrl||DEV_STORE_INVITE),guildId:String(settings.guildId||DEV_STORE_GUILD_ID)})});
app.post('/api/dev-store/remind-later',async(req,res)=>{const c=await current(req);if(!c.x)return res.status(401).json({error:'UNAUTHENTICATED'});const uid=String(c.x.id);data.devStoreTokens=data.devStoreTokens&&typeof data.devStoreTokens==='object'?data.devStoreTokens:{};data.devStoreTokens[uid]={...(data.devStoreTokens[uid]||{}),lastPromptAt:Date.now()};await save().catch(()=>{});res.json({ok:true,nextAt:new Date(Date.now()+Math.max(1,Number(data.settings?.devStore?.promptIntervalHours||3))*3600000).toISOString()})});
app.put('/api/admin/dev-store/settings',async(req,res)=>{const c=await requireAdmin(req,res,'manage_dev_store');if(!c)return;try{const currentSettings=data.settings?.devStore||{};const inviteUrl=String(req.body?.inviteUrl||currentSettings.inviteUrl||DEV_STORE_INVITE).trim();const hours=Math.max(1,Math.min(168,Number(req.body?.promptIntervalHours||3)));if(!/^https?:\/\//i.test(inviteUrl))return res.status(400).json({error:'INVALID_INVITE_URL'});data.settings.devStore={...currentSettings,guildId:DEV_STORE_GUILD_ID,inviteUrl,promptIntervalHours:hours,promptResetAt:Number(currentSettings.promptResetAt||0)};audit(c,'UPDATE_DEV_STORE_SETTINGS','development-store','تعديل إعدادات متجر التطوير');await saveDurable();res.json({ok:true,devStore:data.settings.devStore})}catch(e){console.error('dev-store settings failed:',e);res.status(503).json({error:'STORAGE_ERROR'})}});
