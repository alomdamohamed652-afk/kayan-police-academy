...
async function persistAttemptSafe(attempt){
  ...
  try{await saveExamAttempt(attempt);return {ok:true};}
  catch(error){
    // Keep the in-memory attempt intact and queue retries. The client receives
    // a retryable response instead of losing its answer state.
