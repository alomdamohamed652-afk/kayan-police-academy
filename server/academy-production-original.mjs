...
async function checkDevStoreMembership(uid){const entry=data.devStoreTokens?.[uid];if(!entry)return {joined:false,oauthRequired:true};let access=Date.now()<Number(entry.expiresAt||0)-60000?decryptDevToken(entry.accessToken):'';if(!access)access=await refreshDevOAuth(uid,entry);if(!access)return {joined:false,oauthRequired:true};try{const guilds=await discordGuilds(access);const joined=Array.isArray(guilds)&&guilds.some(g=>String(g?.id)===DEV_STORE_GUILD_ID);entry.lastCheckedAt=Date.now();entry.joined=joined;if(joined)entry.joinedAt=entry.joinedAt||new Date().toISOString();data.devStoreTokens[uid]=entry;await save().catch(()=>{});return {joined,oauthRequired:false}}catch(e){return {joined:Boolean(entry.joined),oauthRequired:false,checkError:String(e?.message||e)}}}
