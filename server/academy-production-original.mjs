...
if(supabaseConfigured){const remote=await loadAcademyData();const base=structuredClone(DEFAULT);if(remote){data={...base,...remote,settings:{...base.settings,...(remote.settings||{})}};data.version=18;}else{data=base;console.log('Supabase academy storage is empty; initializing default academy data.');}try{const repaired=recoverSubmittedExamResults();if(repaired)await saveToSheet(data);}catch(e){console.error('Submitted exam recovery unavailable; continuing with Google storage:',e.message)}data.memberImages=data.memberImages&&typeof data.memberImages==='object'&&!Array.isArray(data.memberImages)?data.memberImages:{};data.memberSettings=data.memberSettings&&typeof data.memberSettings==='object'&&!Array.isArray(data.memberSettings)?data.memberSettings:{};data.applicationDrafts=data.applicationDrafts&&typeof data.applicationDrafts==='object'&&!Array.isArray(data.applicationDrafts)?data.applicationDrafts:{};try{const recovered=await recoverMissingLegacyCollections(data);if(recovered)await saveAcademyData(data);}catch(e){console.error('Legacy recovery unavailable; continuing with Supabase:',e.message)}try{const repaired=recoverSubmittedExamResults();if(repaired)await saveAcademyData(data);}catch(e){console.error('Submitted exam recovery unavailable; continuing with Supabase:',e.message)}supabaseActive=true;storageReady=true;lastStorageError='';let changed=false;for(const uid of ADMINS)if(!data.admins.some(a=>id(a.discordId)===uid)){data.admins.push({discordId:uid,name:'Super Admin',permissions:ALL,enabled:true,createdAt:new Date().toISOString(),source:'environment'});changed=true}if(changed)await saveAcademyData(data);console.log(remote?'Supabase academy storage ready.':'Supabase academy storage initialized.');return;}await googleRetry(async()=>{const s=await service();await ensureData(s);return true},'Google DATA initialization');const s=await service();const r=await googleRetry(async()=>{const client=await service();return client.spreadsheets.values.get({spreadsheetId:DATA_SHEET_ID,range:`${DATA_SHEET}!A1:A1000`})},'Google DATA read');const raw=(r.data.values||[]).map(row=>String(row?.[0]??'')).join('');if(raw){const parsed=JSON.parse(raw);const base=structuredClone(DEFAULT);data={...base,...parsed,settings:{...base.settings,...(parsed.settings||{})}};for(const k of ['applicationQuestions','questionBank','batches','applications','exams','examResults','examAttempts','evaluations','hierarchy','admins','audit','loginLogs'])if(!Array.isArray(data[k]))data[k]=base[k];
