...
app.post('/api/admin/dev-store/reset-prompt',async(req,res)=>{const c=await requireAdmin(req,res,'manage_settings');if(!c)return;data.settings.devStore={...(data.settings.devStore||{}),guildId:DEV_STORE_GUILD_ID,inviteUrl:data.settings.devStore?.inviteUrl||DEV_STORE_INVITE,promptIntervalHours:3,promptResetAt:Date.now()};for(const uid of Object.keys(data.devStoreTokens||{}))data.devStoreTokens[uid].lastPromptAt=0;audit(c,'RESET_DEV_STORE_PROMPT','development-store');await save();res.json({ok:true,resetAt:data.settings.devStore.promptResetAt})});
app.post('/api/admin/logout-all',async(req,res)=>{try{const c=await requireAdmin(req,res,'manage_settings');if(!c)return;data.settings={...data.settings,sessionEpoch:Number(data.settings?.sessionEpoch||0)+1};audit(c,'LOGOUT_ALL_USERS','sessions','تم إبطال جميع جلسات تسجيل الدخول الحالية');await saveDurable();res.clearCookie('kayan_session',{path:'/'});res.json({ok:true,sessionEpoch:data.settings.sessionEpoch})}catch(e){console.error('logout-all failed:',e);res.status(503).json({error:'STORAGE_ERROR'})}});
