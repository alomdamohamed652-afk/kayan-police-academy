...
async function saveAcademyData(data){
  ...
  // Badges are academy configuration data too. Keep them inside the existing
  // academy_settings JSON payload so no destructive schema migration is needed.
  const academySettings={
    ...(data.settings||{}),
    __academyBadges:cleanRows(data.badges),
    __academyMemberBadges:json(data.memberBadges)
  };
  await upsert('academy_settings',[settingsRow(academySettings)],'id');
